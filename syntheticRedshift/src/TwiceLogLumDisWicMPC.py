def TwiceLogLumDisWicMPC(zplus1):
    ...
    LIGHT_SPEED = 3E5;                                       # LIGHT_SPEED is the speed of light (Km/s).
    ...
    TWICE_OMEGA_DE_OVER_OMEGA_DM = 2.0 * OMEGA_DE / OMEGA_DM;
    ALPHA0 = 1.0 + TWICE_OMEGA_DE_OVER_OMEGA_DM;
    X0 = log( ALPHA0 + sqrt((ALPHA0**2)-1.0) );
    X0Sq = X0**2;
    PSI_COEF1 = 2**(2.0/3.0);
    PSI_COEF2 = -PSI_COEF1 / 252.0;
    PSI_COEF3 = +PSI_COEF1 / 21060.0;
    ...
    alpha1          = 1.0 + TWICE_OMEGA_DE_OVER_OMEGA_DM / zplus1**3;
    x1              = log( alpha1 + sqrt( alpha1**2 - 1.0 ) );
    x1Sq            = x1**2;
    psix1           = x1**0.33333333 * ( PSI_COEF1 + x1Sq * (PSI_COEF2 + x1Sq*PSI_COEF3) );
    twicelogLumDisWicMPC = 2*(LOG_NORM_FAC +  log(zplus1*(PSIX0-psix1)));
    return twicelogLumDisWicMPC