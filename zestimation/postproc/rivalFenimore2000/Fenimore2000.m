...
% read Yonetoku data
Rival = importdata('../../in/RedshiftFenimore2000.txt');
...
icolRival = 5; % the column number of Band 2004 redshift predictions
...
for imodel = 1:nModelSFR
    ...
    ylabel('Predicted Redshift: Fenimore (2000)', 'fontSize', fontSize)
    xlim(RangeZ);
    ylim(RangeZ);
    set(gca,'xscale','log')
    set(gca,'yscale','log')
    legend  ( { 'equality line' , [num2str(length(dy)),' BATSE LGRBs'] } ...
            , 'location' , 'northwest' , 'fontSize' , fontSize , 'color' , 'none' ...
            )
    %legend boxoff;
    rho = sprintf('%0.2f', corr(log(dx),log(dy)));
    rhoPos = [45,0.07]; %[0.1,15];
    text(rhoPos(1),rhoPos(2),['\rho = ',num2str(rho)],'HorizontalAlignment','right','Interpreter','tex', 'fontSize' , fontSize )
    set(gca,'color','none')
    set(gcf,'color','none')
